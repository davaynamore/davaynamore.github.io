...
const formNode = document.querySelector('.contact-form');

// console.dir(formNode.elements);

// for(let i = 0; i < formNode.elements.length; i++) {
// 	console.log(formNode.elements[i]);
// }
...
formNode.addEventListener('change', (event) => {
	...
	const arr = [];
	Array.from(formNode.elements).map((el) => {
		if ( el.name !== 'checkbox' && el.name !== 'submit' ) {
